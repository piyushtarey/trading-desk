...
│       ├── tickMarket → markPositions → markPerps
│       ├── scoutPhase → analystPhase → executorPhase (synchronous, same tick)
...
- Client-only boot: `state` starts `null`, `createInitialState(Date.now())` runs in `useEffect` (`createMarket` + `seedHistory` + `createJobs` + `createAgentRuntimes`), rendering `<BootScreen/>` until ready (avoids hydration mismatch).
- Two intervals:
  - `TICK_MS` (2s): if `!running`, only `now` updates; else deep-clone slices and call `runTick(next)`.
  - `POLL_MS` (15s): `fetch("/api/prices", {cache:"no-store"})` → `applyLivePrices` → set `marketSource/lastLiveAt`; errors swallowed (sim continues).

## Data flow

1. **Market** — `lib/market.ts`: 12 `PAIR_SEEDS` → `createMarket(now)` builds `MarketPair`s with 24-pt spark; `tickMarket(pairs)` random-walks every tick (global shove ±0.1%, per-pair `0.0012 + volatility*0.004`, 3% shock ±1%).
2. **Live blend** — `lib/prices.ts`: `COIN_TO_PAIR` maps 12 CoinGecko IDs → pair IDs. Direct hits update price/prev/change/spark/volume/liquidity/volatility; synthetic crosses computed as `JUP/SOL = JUPUSD/SOLUSD`, `BONK/SOL = BONKUSD/SOLUSD`, `WBTC/ETH = WBTCUSD/ETHUSD`. First live hit sets `prevPrice = hit` to avoid a fake flash. Uncovered pairs keep simulating.
